...
function getProbabilities(batDiff: number, isPowerplay: boolean, isDeath: boolean, ballsFaced: number): number[] {
  ...
  const weights = [45, 32, 6, 1, 10, 3, 3]; // [Dot, 1, 2, 3, 4, 6, W]
...
export function simulateMatchEngine(team1: SimTeam, team2: SimTeam) {
  ...
  const battingFirstTeam = tossDecision === 'BAT' ? tossWinnerTeam : tossLoserTeam;
  const bowlingFirstTeam = tossDecision === 'BAT' ? tossLoserTeam : tossWinnerTeam;
