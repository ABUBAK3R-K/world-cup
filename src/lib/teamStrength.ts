...
function getPositionLabel(pos: number): 'TOP' | 'MID' | 'LOW' {
  if (pos <= 3) return 'TOP';
  if (pos <= 7) return 'MID';
  return 'LOW';
}
...
export function calculateTeamStrength(squad: DraftedPlayer[], battingOrder: BattingOrder[]): TeamStrength {
  ...
  orderedPlayers.forEach((player, index) => {
    ...
    if (player.battingOrderType !== posLabel) {
      battingMultiplier = 0.8; // 20% penalty for out-of-position
      issues.push(`${player.player.name} is a ${player.battingOrderType} order batter at #${position} (${posLabel})`);
    }
