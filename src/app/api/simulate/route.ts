...
function buildPlayerTeam(squad: DraftedPlayer[], battingOrder: BattingOrder[]): SimTeam {
  ...
  const simPlayers: SimPlayer[] = orderedSquad.map(p => {
    ...
    if (p.battingOrderType === 'TOP' && assignedPos > 3) isOutOfPos = true;
    if (p.battingOrderType === 'MID' && (assignedPos < 4 || assignedPos > 7)) isOutOfPos = true;
    if (p.battingOrderType === 'LOW' && assignedPos < 8) isOutOfPos = true;
...
export async function POST(req: Request) {
  try {
    ...
    let quarterFinals: MatchResult[] = [];
    let semiFinals: MatchResult[] = [];
